...
from pykytdownloader import ytDownloader
...
# Popup Youtube Search dialog
class YoutubeSearchWindow(wx.Frame):
    def __init__(self, parent, mainWindow, id, title, KaraokeMgr):
        wx.Frame.__init__(self, parent, wx.ID_ANY, title, style=wx.DEFAULT_FRAME_STYLE | wx.FRAME_FLOAT_ON_PARENT)
        ...
        self._HelpText = wx.StaticText(self.panel, wx.ID_ANY,
                                       "Enter artist and title to download the most popular karaoke song from Youtube!\n",
                                       style=wx.ALIGN_CENTER)
        # Create the artist
        self.Artist = ""
        self.ArtistText = wx.StaticText(self.panel, wx.ID_ANY, "Artist: ")
        self.ArtistTextCtrl = wx.TextCtrl(self.panel, -1, value=str(self.Artist))

        # Create the title
        self.SongTitle = ""
        self.SongTitleText = wx.StaticText(self.panel, wx.ID_ANY, "Title: ")
        self.SongTitleTextCtrl = wx.TextCtrl(self.panel, -1, value=str(self.SongTitle))

        # Create specify URL text box
        self.specifyURLID = wx.NewId()
        self.specifyURLCheckBox = wx.CheckBox(self.panel, self.specifyURLID, "Specify URL: ")
        self.specifyURLCheckBox.SetValue(False)
        self.specURLSizer = wx.BoxSizer(wx.HORIZONTAL)
        self.specURLSizer.Add(self.specifyURLCheckBox, 0, wx.TOP, 9)
        wx.EVT_CHECKBOX(self, self.specifyURLID, self.SpecURLChanged)

        # Create the URL
        self.URL = ""
        self.URLTextCtrl = wx.TextCtrl(self.panel, -1, value=str(self.URL))
        self.URLTextCtrl.Enable(False)

        # Create confirmation buttons
        self.hsizer = wx.BoxSizer(wx.HORIZONTAL)
        b = wx.Button(self.panel, wx.ID_OK, 'OK')
        self.Bind(wx.EVT_BUTTON, self.clickedOK, b)
        self.hsizer.Add(b, flag=wx.EXPAND | wx.RIGHT | wx.LEFT, border=10)

        c = wx.Button(self.panel, wx.ID_CANCEL, 'Cancel')
        self.Bind(wx.EVT_BUTTON, self.clickedCancel, c)
        self.hsizer.Add(c, flag=wx.EXPAND | wx.RIGHT, border=10)

        # Create the grid to layout controls
        self.LowerSizer = wx.BoxSizer(wx.VERTICAL)
        self.LowerSizer.Add(self.ArtistText, 0, wx.RIGHT | wx.ALIGN_CENTER_VERTICAL | wx.ALL, 3)
        self.LowerSizer.Add(self.ArtistTextCtrl, 1, wx.EXPAND | wx.ALL, 3)
        self.LowerSizer.Add(self.SongTitleText, 0, wx.RIGHT | wx.ALIGN_CENTER_VERTICAL | wx.ALL, 3)
        self.LowerSizer.Add(self.SongTitleTextCtrl, 1, wx.EXPAND | wx.ALL, 3)
        self.LowerSizer.Add(self.specURLSizer, 1, wx.EXPAND | wx.ALL, 3)
        self.LowerSizer.Add(self.URLTextCtrl, 1, wx.EXPAND | wx.ALL, 3)

        # Create the main sizer
        self.MainSizer = wx.BoxSizer(wx.VERTICAL)
        self.MainSizer.Add(self._HelpText, 0, wx.EXPAND | wx.ALL, 6)
        self.MainSizer.Add(self.LowerSizer, 0, wx.EXPAND, 3)
        self.MainSizer.Add(self.hsizer, 0, flag=wx.ALIGN_CENTER | wx.TOP | wx.BOTTOM, border=10)
    ...
    # User changed the specify URL checkbox, enable the text control appropriately
    def SpecURLChanged(self, event):
        self.URLTextCtrl.Enable(self.specifyURLCheckBox.IsChecked())
        if not self.specifyURLCheckBox.IsChecked():
            self.URLTextCtrl.Value = ""
            
    def IsValidatedInput(self):
        self.Artist = self.ArtistTextCtrl.Value.strip()
        self.SongTitle = self.SongTitleTextCtrl.Value.strip()
        self.URL = self.URLTextCtrl.Value.strip()
        
        if not self.Artist:
            answer = wx.MessageBox("Please enter an artist name.", "Warning", wx.OK | wx.ICON_WARNING)
            return False
        elif not self.SongTitle:
            answer = wx.MessageBox("Please enter a song title.", "Warning", wx.OK | wx.ICON_WARNING)
            return False
        elif self.specifyURLCheckBox.IsChecked() and not self.URL:
            answer = wx.MessageBox("Please enter the Youtube video URL.", "Warning", wx.OK | wx.ICON_WARNING)
            return False
        return True

    def clickedOK(self, event):
        if self.IsValidatedInput():
            if ytDownloader.SongExistsOnDisk(self.Artist, self.SongTitle):
                warningString = "Song exists on disk.  Overwrite?"
                answer = wx.MessageBox(warningString, "Overwrite existing song?", wx.YES_NO | wx.ICON_QUESTION)
                if answer == wx.YES:
                    self.downloadVideo()
            else:        
                self.downloadVideo()
    ...
    def downloadVideo(self):
        ...
        busyDlg = wxBusyCancelDialog(self.KaraokeMgr.Frame, "Downloading")
        isSuccess = ytDownloader.DownloadKaraokeSong(self.KaraokeMgr, self.Artist, self.SongTitle, self.URL,
        wxAppYielder(), busyDlg)
                
        if not isSuccess:
            answer = wx.MessageBox(ytDownloader.ErrorReason, "Warning", wx.OK | wx.ICON_WARNING)
        else:
            self.mainWindow.ScanFolders()            
...
class DatabaseSetupWindow(wx.Frame):
    def __init__(self, parent, mainWindow, id, title, KaraokeMgr):
        ...
        # Create the Add From Youtube
        self.GetYoutubeText = wx.StaticText(self.panel, wx.ID_ANY, "Mass download from Youtube: ")
        self.GetYoutubeButtonID = wx.NewId()
        self.GetYoutubeButton = wx.Button(self.panel, self.GetYoutubeButtonID, "Open CSV file")
        wx.EVT_BUTTON(self, self.GetYoutubeButtonID, self.OnGetYoutubeClicked)
        ...
        if ytDownloader.IsAvailable:
            self.LowerSizer.Add(self.GetYoutubeText, 0, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 3)
            self.LowerSizer.Add(self.GetYoutubeButton, 1, wx.ALL, 3)
    ...
    # User wants to mass download from Youtube
    def OnGetYoutubeClicked(self, event):
        if ytDownloader.IsAvailable:
            ...
            busyDlg = wxBusyCancelDialog(self.KaraokeMgr.Frame, "Downloading")
            isSuccess = ytDownloader.MassDownloadKaraokeSong(self.KaraokeMgr, openFileDialog.GetPath(), 
            wxAppYielder(), busyDlg)
...
class ConfigWindow(wx.Frame):
    ...
    def __layoutCdgPage(self):
        ...
        self.ExcludeNonMatchingCheckBox = wx.CheckBox(panel, -1,
                                                      "Exclude from search results files not matching naming scheme")
    ...
    def clickedOK(self, event):
        ...
        if ((self.SongInfoCheckBox.IsChecked() == settings.CdgDeriveSongInformation)
            ...
            if self.SongInfoCheckBox.IsChecked() or (
...
class ExportWindow(wx.Frame):
    ...
    def __layoutWindow(self):
        ...
        text = wx.StaticText(self.panel, -1,
                             'This will save the entire song list as a file, which can\n'
                             'then be imported into a spreadsheet or other database program.\n')
        vsizer.Add(text, flag=wx.ALIGN_CENTER)
        
        comboBoxLabel = wx.StaticText(self.panel, -1, 'Select an export type:')
        vsizer.Add(comboBoxLabel, flag=wx.ALIGN_LEFT)
        self.ExportTypes = ['Export to artist,title CSV file', 'Export to tab-delimited text file']
        self.ExportTypeSelectBox = wx.ComboBox(self.panel, -1, value=self.ExportTypes[0], choices=self.ExportTypes, style=wx.CB_READONLY, name='Export type: ')
        vsizer.Add(self.ExportTypeSelectBox, flag=wx.TOP, border=10)
        
        self.SameSongOneLineCheckBox = wx.CheckBox(self.panel, -1, 'Write identical artist/title files to one line')
        vsizer.Add(self.SameSongOneLineCheckBox, flag=wx.TOP, border=10)
        
        songDb = self.parent.KaraokeMgr.SongDB
        if not songDb.GotTitles and not songDb.GotArtists:
            # No songs/titles, so can't unify.
            self.SameSongOneLineCheckBox.Enable(False)
    ...
    def clickedOK(self, event):
        ...
        if self.ExportTypeSelectBox.GetValue() == self.ExportTypes[0]:
            self.writeCSV()
        elif self.ExportTypeSelectBox.GetValue() == self.ExportTypes[1]:
            self.writeTabDelimited()
    ...
    def writeCSV(self):
        ...
        for song in songDb.SongList:
            line = song.DisplayFilename.replace(' - ', ',')
            line = line.replace('.mpg', '')
            file.write(line)
            file.write('\n')
    
    def writeTabDelimited(self):
        oneLine = self.SameSongOneLineCheckBox.IsChecked()
    ...
    def PopulateFolder(self, root_node):
        ...
        for item in dir_list:
            ...
            try:
                ...
                node = self.FileTree.AppendItem(root_node, item.encode('UTF-8', 'replace'),
                                                image=self.FolderClosedIconIndex)
...
class SearchResultsPanel(wx.Panel):
    ...
    def __init__(self, parent, mainWindow, id, KaraokeMgr, x, y):
        ...
        self.SearchTimerID = wx.NewId()
        self.SearchTimer = wx.Timer(self, self.SearchTimerID)
        wx.EVT_TIMER(self, self.SearchTimerID, self.OnSearchClicked)
        
        self.SearchTextID = wx.NewId()
        self.SearchText = wx.TextCtrl(self, self.SearchTextID, style=wx.TE_PROCESS_ENTER)
        wx.EVT_KEY_UP(self.SearchText, self.OnSearchTextTyped)
        
        self.ConsiderPreviousSearchText = True
        self.PreviousSearchText = None
        
        # SearchButton should not be needed
        #self.SearchButtonID = wx.NewId()
        #self.SearchButton = wx.Button(self, -1, "Search")
        #wx.EVT_BUTTON(self, self.SearchButtonID, self.OnSearchClicked)
        ...
        # Create Search Youtube button
        self.GetYoutubeButtonID = wx.NewId()
        self.GetYoutubeButton = wx.Button(self, self.GetYoutubeButtonID, "Search Youtube")
        wx.EVT_BUTTON(self, self.GetYoutubeButtonID, self.mainWindow.OnGetYoutubeClicked)
        self.SearchSizer.Add(self.GetYoutubeButton, 0, wx.EXPAND, 5)
        ...
        self.menuFileRenameId = wx.NewId()
        self.menuFileDeleteId = wx.NewId()
        self.menuFileDetailsId = wx.NewId()
    ...
    def OnSearchTextTyped(self, event):
        if self.SearchTimer.IsRunning():
            self.SearchTimer.Stop()
            
        self.SearchTimer.Start(1200, True)
    ...
    def OnSearchClicked(self, event):
        ...
        if self.ConsiderPreviousSearchText and self.SearchText.GetValue() == self.PreviousSearchText:
            return
        elif self.SearchText.GetValue() == "*":
            answer = wx.MessageBox(
                "This will load the entire song database into the search results!\nThis may take a long time to complete depending on the number of songs listed in the database.",
                "Load Database", wx.YES_NO | wx.ICON_QUESTION)
            self.SearchText.SetValue("")
        ...
        self.ConsiderPreviousSearchText = True
        self.PreviousSearchText = self.SearchText.GetValue()
        ...
        if self.KaraokeMgr.SongDB.GetDatabaseSize() == 0:
            ...
            if answer == wx.YES:
                ...
                self.DBFrame = DatabaseSetupWindow(self.parent, self.mainWindow, -1, "Database Setup", self.KaraokeMgr)
                self.StatusBar.SetStatusText("No Search Performed")
            else:
    ...
    def OnColumnClicked(self, event):
        ...
        if column == self.FilenameCol:
            # Sort by filename
            self.ListPanel.SortItems(lambda a, b: cmp(self.SongStructList[a].DisplayFilename.lower(),
                                                      self.SongStructList[b].DisplayFilename.lower()))
        elif column == self.TitleCol:
            # Sort by title
            self.ListPanel.SortItems(
                lambda a, b: cmp(self.SongStructList[a].Title.lower(), self.SongStructList[b].Title.lower()))
        elif column == self.ArtistCol:
            # Sort by artist
            self.ListPanel.SortItems(
                lambda a, b: cmp(self.SongStructList[a].Artist.lower(), self.SongStructList[b].Artist.lower()))
        elif column == self.DiscCol:
            # Sort by disc
            self.ListPanel.SortItems(
                lambda a, b: cmp(self.SongStructList[a].Disc.lower(), self.SongStructList[b].Disc.lower()))
    ...
    def OnRightClick(self, event):
        ...
        if self.ListPanel.GetItemCount() > 0:
            ...
            menu.Append(self.menuFileRenameId, "Rename file")
            wx.EVT_MENU(menu, self.menuFileRenameId, self.OnMenuSelection)
            menu.Append(self.menuFileDeleteId, "Delete file")
            wx.EVT_MENU(menu, self.menuFileDeleteId, self.OnMenuSelection)
            menu.Append(self.menuFileDetailsId, "File Details")
            wx.EVT_MENU(menu, self.menuFileDetailsId, self.OnMenuSelection)
    ...
    def OnMenuSelection(self, event):
        ...
        elif event.GetId() == self.menuFileRenameId:
            inputDialog = wx.TextEntryDialog(self.mainWindow,'New filename:','Rename file', song.DisplayFilename)
            result = inputDialog.ShowModal()
            if result == wx.ID_OK:
                try:
                    newFilepath = song.Filepath.replace(song.DisplayFilename, inputDialog.GetValue())
                    os.rename(song.Filepath, newFilepath)
                    self.mainWindow.ScanFolders()
                    self.ConsiderPreviousSearchText = False
                    self.OnSearchClicked(self)
                except OSError:
                    ErrorPopup("Error renaming file " + song.Filepath)
                    pass
            
            inputDialog.Destroy()
        elif event.GetId() == self.menuFileDeleteId:
            warningString = "Delete '" + song.DisplayFilename + "' from disk?  This cannot be undone."
            answer = wx.MessageBox(warningString, "Delete file from disk?", wx.YES_NO | wx.ICON_QUESTION)
            if answer == wx.YES:
                try:
                    ...
                    ErrorPopup("Error deleting file " + song.Filepath)
                    pass
        elif event.GetId() == self.menuFileDetailsId:
            detailsString = ''
    ...
    def doResize(self, resize_event=False):
        ...
        # settings, but resize the Artist column to match whatever space is left.
            ...
            if not self.KaraokeMgr.SongDB.Settings.CdgDeriveSongInformation:
                ...
                width = (
                        listWidth - discWidth) / 2  # With derived information title and artist are of equal importance.
...
class Playlist(wx.Panel):
    ...
    def doResize(self, resize_event=False):
        ...
        if (resize_event == False):
            ...
            filenameWidth = max(self.MaxFilenameWidth,
                                (len(PLAY_COL_FILENAME) * self.GetCharWidth() + self.GetCharWidth()))
            ...
            performerWidth = max(self.MaxPerformerWidth,
                                 (len(PLAY_COL_PERFORMER) * self.GetCharWidth() + self.GetCharWidth()))
    ...
    def DelItem(self, item_index):
        ...
        resize_needed = False
        if ((len(self.PlaylistSongStructList[item_index][
                     0].DisplayFilename) * self.GetCharWidth()) == self.MaxFilenameWidth):
            resize_needed = True
        ...
        if resize_needed:
            ...
            for song_tuple in self.PlaylistSongStructList:
                if ((len(song_tuple[0].DisplayFilename) * self.GetCharWidth()) > self.MaxFilenameWidth):
                    self.MaxFilenameWidth = len(
                        song_tuple[0].DisplayFilename) * self.GetCharWidth() + self.GetCharWidth()
...
class PrintSongListWindow(wx.Frame):
    ...
    def __layoutWindow(self):
        ...
        label = wx.StaticText(self.panel, -1,
                              'The song database includes %s songs.' % (len(self.KaraokeMgr.SongDB.FullSongList)))
...
class PyKaraokeWindow(wx.Frame):
    def __init__(self, parent, id, title, KaraokeMgr):
        ...
        if manager.settings.SplitVertically:
            self.splitter.SplitVertically(self.leftPanel, self.rightPanel, sashPosition=size[0] / 2)
        else:
            self.splitter.SplitHorizontally(self.leftPanel, self.rightPanel, size[0] / 2)
    ...
    def OnKamikazeClicked(self, event):
        ...
        if self.KaraokeMgr.SongDB.GetDatabaseSize() == 0:
            ...
            if answer == wx.YES:
                # Open up the database setup dialog
                self.Frame = DatabaseSetupWindow(self, self, -1, "Database Setup", self.KaraokeMgr)
        else:
    ...
    def OnClearPlaylistClicked(self, event):
        ...
        # User wants to get from Youtube
    def OnGetYoutubeClicked(self, event):
        if ytDownloader.IsAvailable:
            self.Frame = YoutubeSearchWindow(self, self, -1, "Search Youtube", self.KaraokeMgr)
        else:
            answer = wx.MessageBox(ytDownloader.ErrorReason, "Warning", wx.OK | wx.ICON_WARNING)
    ...
    def handleIdle(self, event):
        ...
        if self.Player:
            ...
            if self.gui:
                ...
                if timeLeft <= 0:
                    self.Frame.PlaylistPanel.StatusBar.SetStatusText(
                        "[%02d:%02d] %s - %s" % (minutes, seconds, self.Player.Song.Artist, self.Player.Song.Title))
                else:
                    minutesRemaining = timeLeft / 60
                    secondsRemaining = timeLeft % 60
                    self.Frame.PlaylistPanel.StatusBar.SetStatusText("[%02d:%02d/%02d:%02d] %s - %s" % (
                    minutes, seconds, minutesRemaining, secondsRemaining, self.Player.Song.Artist,
                    self.Player.Song.Title))
